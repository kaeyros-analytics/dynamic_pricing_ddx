...
# # select a machine-type  (180 obse --> from 2005 to 2019)
# check_it <- input_data_1 %>%
#   dplyr::filter(machine_id == "M_001" & client_id == "client_9") #%>%
#  # dplyr::select(-id, -machine_id, -date, -date_month) 
...
table(temp5$client_id) # 180 data_rows for each client
...
clients <- levels(temp6$client_id)
nreg <- length(clients)
